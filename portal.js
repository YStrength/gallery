document.addEventListener('DOMContentLoaded', function() {
    ...
    const sidebarContainer = document.getElementById('sidebar-container'),
          ...
          progressOverlay = document.getElementById('download-progress-overlay'),
    ...
    function loadAlbum(categoryName, albumName, clickedLi) {
        ...
        currentAlbumName = albumName; // 保存当前相册名
        currentPage = 1; galleryView.welcome.classList.add('hidden');
        ...
        clickedLi.classList.add('active'); renderGallery();
    ...
    function openLightbox(src) { document.body.classList.add('lightbox-open'); lightboxOverlay.innerHTML = ''; const img = document.createElement('img'); img.src = src; img.style.position = 'absolute'; img.onload = () => { lightboxOverlay.appendChild(img); lightboxOverlay.classList.add('visible'); setupLightboxInteraction(img); }; lightboxOverlay.addEventListener('click', (e) => { if (e.target === lightboxOverlay) closeLightbox(); }); lightboxOverlay.addEventListener('dblclick', (e) => { if (e.target === lightboxOverlay) closeLightbox(); }); }
    ...
    function setupLightboxInteraction(img) { let matrix = [1, 0, 0, 1, 0, 0], lastTouch = { x: 0, y: 0, time: 0, distance: 0 }, isDragging = false, isPinching = false; const fitScale = Math.min(window.innerWidth / img.naturalWidth, window.innerHeight / img.naturalHeight) * 0.95; matrix[0] = matrix[3] = fitScale; matrix[4] = (window.innerWidth - img.naturalWidth * fitScale) / 2; matrix[5] = (window.innerHeight - img.naturalHeight * fitScale) / 2; applyTransform(false); function applyTransform(useTransition = true) { img.style.transform = `matrix(${matrix.join(',')})`; img.style.transition = useTransition && !isDragging && !isPinching ? 'transform 0.3s ease' : 'none'; } function getDistance(p1, p2) { return Math.hypot(p1.clientX - p2.clientX, p1.clientY - p2.clientY); } function getMidpoint(p1, p2) { return { x: (p1.clientX + p2.clientX) / 2, y: (p1.clientY + p2.clientY) / 2 }; } function onPointerDown(e) { e.preventDefault(); isDragging = true; const point = e.touches ? e.touches[0] : e; lastTouch.x = point.clientX; lastTouch.y = point.clientY; if (e.touches) { const now = Date.now(); if (now - lastTouch.time < 300) { handleDoubleClick(e.touches[0]); } lastTouch.time = now; } if (e.touches && e.touches.length === 2) { isPinching = true; lastTouch.distance = getDistance(e.touches[0], e.touches[1]); } } function onPointerMove(e) { if (!isDragging) return; e.preventDefault(); if (isPinching && e.touches && e.touches.length === 2) { const newDist = getDistance(e.touches[0], e.touches[1]); const scaleRatio = newDist / lastTouch.distance; lastTouch.distance = newDist; const midpoint = getMidpoint(e.touches[0], e.touches[1]); zoom(scaleRatio, midpoint); } else if (!isPinching) { const point = e.touches ? e.touches[0] : e; const dx = point.clientX - lastTouch.x; const dy = point.clientY - lastTouch.y; matrix[4] += dx; matrix[5] += dy; lastTouch.x = point.clientX; lastTouch.y = point.clientY; applyTransform(false); } } function onPointerUp(e) { isDragging = false; if (e.touches && e.touches.length < 2) isPinching = false; if (!e.touches || e.touches.length === 0) isPinching = false; } function handleDoubleClick(point) { let currentScale = matrix[0]; let nextScale = (Math.abs(currentScale - 1.0) < 0.05) ? fitScale : 1.0; const scaleRatio = nextScale / currentScale; zoom(scaleRatio, { x: point.clientX, y: point.clientY }, true); } function zoom(scaleRatio, center, useTransition = false) { const currentScale = matrix[0]; let newScale = currentScale * scaleRatio; newScale = Math.max(0.2, Math.min(newScale, 8)); scaleRatio = newScale / currentScale; if (scaleRatio === 1) return; const newTx = center.x - (center.x - matrix[4]) * scaleRatio; const newTy = center.y - (center.y - matrix[5]) * scaleRatio; matrix[0] = newScale; matrix[3] = newScale; matrix[4] = newTx; matrix[5] = newTy; applyTransform(useTransition); } img.addEventListener('mousedown', onPointerDown); img.addEventListener('touchstart', onPointerDown, { passive: false }); window.addEventListener('mousemove', onPointerMove, { passive: false }); window.addEventListener('touchmove', onPointerMove, { passive: false }); window.addEventListener('mouseup', onPointerUp); window.addEventListener('touchend', onPointerUp); img.addEventListener('dblclick', function(e) { e.preventDefault(); e.stopPropagation(); handleDoubleClick(e); }); img.addEventListener('wheel', function(e) { e.preventDefault(); const scaleRatio = e.deltaY < 0 ? 1.1 : 1 / 1.1; zoom(scaleRatio, { x: e.clientX, y: e.clientY }); }, { passive: false }); }
    ...
    async function downloadImagesAsZip(imageUrls, zipFileName) {
        ...
        for (const imageUrl of imageUrls) {
            ...
            progressText.textContent = `正在下载: ${downloadedCount} / ${imageUrls.length}`;
    ...
    function setupEventListeners() {
        ...
        downloadCurrentBtn.addEventListener('click', () => {
             ...
             const imageUrls = currentAlbumImages.slice(startIndex, endIndex).map(img => img.src);
             downloadImagesAsZip(imageUrls, `${currentAlbumName}-Page${currentPage}`);
        });

        downloadAllBtn.addEventListener('click', () => {
            const imageUrls = currentAlbumImages.map(img => img.src);
            downloadImagesAsZip(imageUrls, `${currentAlbumName}-All`);
        });
    }
